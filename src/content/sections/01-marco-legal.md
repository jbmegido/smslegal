---
orden: 1
titulo: "I. Marco Normativo Aplicable"
tipo: "marco-legal"
tonalidad: "informe-riesgos"
backgroundColor: "alt"
showSeparator: true
---

## I. Marco Normativo Aplicable

El envío de comunicaciones comerciales por SMS en España está regulado por:

- **Reglamento General de Protección de Datos (RGPD)** - UE 2016/679
- **Ley Orgánica 3/2018 de Protección de Datos Personales y garantía de los derechos digitales (LOPDGDD)**
- **Ley 34/2002 de Servicios de la Sociedad de la Información y de Comercio Electrónico (LSSI)**
- **Real Decreto 1720/2007** sobre Lista Robinson

### Obligaciones del remitente

1. Consentimiento previo, específico e informado del destinatario
2. Identificación clara del remitente
3. Facilitar mecanismo de baja efectivo (BAJA/STOP)
4. Registro de consentimientos con trazabilidad temporal
5. Verificación Lista Robinson para comunicaciones no transaccionales

### Infracciones y sanciones

La Agencia Española de Protección de Datos (AEPD) puede imponer sanciones de hasta **20 millones de euros** o el **4% de la facturación anual global** del ejercicio anterior (lo que sea mayor) por vulneraciones graves del RGPD.

El envío no consentido de comunicaciones comerciales se considera infracción grave según el artículo 38.3.d) de la LSSI, con multas de hasta **150.000 euros**.